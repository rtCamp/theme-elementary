...
If no blocks exist yet, the build is skipped and no empty output folder is created.
...
| `build:block-manifest` fails on Windows | Old shell-based command used Unix-only syntax (`find`, `grep`, `rm -f`) | Ensure `build:block-manifest` points to `node ./bin/build-block-manifest.js` in `package.json` |
